...
class LLaMA_adapter(nn.Module):
    ...
    def forward_audio(self, inputs, cache_size=10, cache_t=20, cache_weight=0.5):
        #print(len(inputs["Audio"]))
        #print(inputs["Audio"][1])
        #print(type(inputs["Audio"][0]))
        #print(inputs["Audio"][0].shape)
        outputs = []
        outputs_weights = []
    ...
    @torch.inference_mode()
    def forward_inference_no_audio(self, tokens, start_pos: int):
        _bsz, seqlen = tokens.shape
        h = self.llama.tok_embeddings(tokens)
        freqs_cis = self.llama.freqs_cis.to(h.device)
        freqs_cis = freqs_cis[start_pos:start_pos + seqlen]
        ...
        for layer in self.llama.layers[-1 * self.query_layer:]:
            # print(f"Layer {l}")
            h = layer(h, start_pos, freqs_cis, mask, prefix_query[prefix_index].repeat(_bsz, 1, 1))
            prefix_index = prefix_index + 1
            l = l + 1
    ...
    def generate_with_audio_query(
        ...
        # print(f"Bsz: {bsz}")
        params = self.llama.params
        # print(type(params.max_batch_size))
        # print(f"Params.max_batch_size: {params.max_batch_size}")
        assert bsz <= params.max_batch_size, (bsz, params.max_batch_size)
    ...
    def generate_no_audio(
        ...
        for cur_pos in range(start_pos, total_len):
            with torch.cuda.amp.autocast():
                logits = self.forward_inference_no_audio(tokens[:, prev_pos:cur_pos], prev_pos)
            if temperature > 0:
                probs = torch.softmax(logits / temperature, dim=-1)
                next_token = sample_top_p(probs, top_p)
            else:
