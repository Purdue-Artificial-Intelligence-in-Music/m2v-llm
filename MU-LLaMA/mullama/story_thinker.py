...
class StoryThinker(threading.Thread):
    ...
    def sd_summarize_call(self, ecphory, music_info):
        ...
        sd_prompt = self.llama_inference(self.sd_summary + "\"" + full_info + "\" Your prompt is: \"", max_length=2500)
    ...
    def process_file(self, prompts_folder, prompts_name, chunk):
        ...
        if self.output_dir is not None:
            out_list.save(os.path.join(self.output_dir, f"{prompts_name}_st_chunk_{chunk}.hlst"))
            self.outputs.append((self.output_dir, prompts_name, chunk))
        else:
            out_list.save(os.path.join(prompts_folder, f"{prompts_name}_st_chunk_{chunk}.hlst"))
